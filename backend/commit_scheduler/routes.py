...
@router.post("")
async def create_job(payload: CommitJobCreate, org_id: str = Depends(get_current_org_id)):
...
@router.patch("/{job_id}")
def update_job(job_id: str, payload: CommitJobUpdate, org_id: str = Depends(get_current_org_id)):
...
async def list_repos_for_org(provider: str = "github", org_id: str = Depends(get_current_org_id)):
...
async def list_branches_for_repo(repo_full_name: str, provider: str = "github", org_id: str = Depends(get_current_org_id)):
    access_token = service._get_github_token_for_org(org_id)
    vcs = git_ops.get_provider(provider)
...
def add_files(job_id: str, files: list[CommitJobFile], org_id: str = Depends(get_current_org_id)):
...
def list_files(job_id: str, org_id: str = Depends(get_current_org_id)):
    service.get_job_or_404(job_id, org_id)
